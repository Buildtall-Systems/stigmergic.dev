...
func TestWikilinkIntegrationResolved(t *testing.T) {
	...
	r := NewTreeResolver(testFiles())
	html, err := Parse([]byte("See [[Simple Note]] for details."), r)
	if err != nil {
...
func TestWikilinkIntegrationUnresolved(t *testing.T) {
	...
	r := NewTreeResolver(testFiles())
	html, err := Parse([]byte("See [[Missing Page]] for details."), r)
	if err != nil {
		t.Fatalf("Parse failed: %v", err)
	}
...
func TestWikilinkIntegrationAlias(t *testing.T) {
	...
	r := NewTreeResolver(testFiles())
	html, err := Parse([]byte("See [[Simple Note|my note]] for details."), r)
	if err != nil {
		t.Fatalf("Parse failed: %v", err)
	}
...
func TestWikilinkIntegrationNilResolver(t *testing.T) {
	...
	html, err := Parse([]byte("Text with [[brackets]] here."), nil)
	if err != nil {
		t.Fatalf("Parse failed: %v", err)
	}
