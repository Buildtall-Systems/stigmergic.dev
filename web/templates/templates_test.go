...
func TestHomeRendersWithoutTree(t *testing.T) {
	...
	err := Home(nil, "/test/path", testTheme(), []models.SearchableFile{}).Render(context.Background(), &sb)
	if err != nil {
...
func TestHomeRendersWithEmptyTree(t *testing.T) {
	...
	tree := &models.Tree{}
	var sb strings.Builder
	err := Home(tree, "/test/path", testTheme(), []models.SearchableFile{}).Render(context.Background(), &sb)
	if err != nil {
		t.Fatalf("failed to render: %v", err)
	}
